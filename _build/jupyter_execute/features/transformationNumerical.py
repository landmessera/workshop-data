...
* Eingabefehler: wenn z.B. Menschen ein Fehler bei der Eingebe von Hand gemacht haben)
* Messfehler: beim Aufzeichnen von Daten ensteht ein Fehler
...
Wobei der Faktor variabel ist. Standardeinstellung beträgt 1,5.
...
Die Isolation Forest Methode und das DBScn Clustering können angewendet werden, wenn die Ausreißererkunng auf mehrdimensionalen Daten durchgeführt werden soll. Im Fall des Titanic Datenset werden die einzelnen Merkmale getrennt betrachtet, daher handelt es sich nicht um multivariate sondern um univariate Daten.
...
* Die Anzahl der Geschwister oder Ehepartner reicht von 0 und 8
...
### Min-Max-Skalierung
Die Min-Max-Skalierung, auch Normalisierung genannt, ist eine Verschiebung der Werte in den Wertebereich von 0 und 1. Hierzu wird zuerst der Minimalwert von allen Werten subtrahiert und anschließend alle Werte durch die Differenz von dem Maximal- und Minimalwert geteilt. 
...
Die Diskretisierung, auch Binning genannt, transformiert kontinuierliche in diskrete Werte. Hierzu wird ein Set von zusammenhängenden Intervallen definiert, sogenannte Bins (Behälter). Jeder Behälter erhälte einen Namen oder eine Regel wie die Elemente dieser Behälter benannt werden, z.B. der Mittelwert der enthaltenen Elemente. Anschließend wird jeder kontinuierliche Wert einem der Intervalle bzw. Bins zugewiesen und mit der Bezeichnung ersetzt.
...
print('Mittelwert der Elemente der Bins: ', statistics.mean(bin1), statistics.mean(bin2), statistics.mean(bin3))
...
Die letzten fünf Zeilen der skalierten Werte:
...
Bei der Anwendung einer Diskretisierung ist zu beachten, dass Informationen verloren gehen. Das kann von Vorteil sein, wenn die verlorenen Informationen für den Anwendungsfall relevant sind, bzw. im Gegenteil, störend sind. Ist das der Fall, bewirkt man mit der Diskretisierung ein Smoothing-Effekt. Rauschen wird entfernt. Außerdem kann eine Diskretisierung eine vereinfachte Interpretation der Daten bewirken. Bewirkt eine Diskretisierung jedoch einen Verlust von relevanten Informationen, wirkt sich das negativ auf das Endergebnis aus. Mit Domänenwissen kann zunächst abgeschätzt werden, ob eine Diskretisierung erfolgsversprechend ist. Am Ende muss jedoch immer eine Erprobung stattfinden um sicher gehen zu können, dass eine Diskretisierung die Performance des Algorithmus steigert. Hierzu führt man Experimente mit unterschiedlichen Transformationsmöglichkeiten durch und analysiert das Ergebnis. Mehr dazu im Abschnitt "Feature Engineering". 
